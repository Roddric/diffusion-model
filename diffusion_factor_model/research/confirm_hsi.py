"""One-time 2024-current external evaluation of the frozen HSI dual pools."""
...
from research.confirm_ftse100 import _composite_ratio
...
POOL_NAMES = {
    "gaussian_base_pool": "HSI-Gaussian-Base-Pool",
    "student_t_base_pool": "HSI-Student-t-Base-Pool",
}
...
def run(args):
    ...
    if output.exists():
        raise FileExistsError(
            f"HSI evaluation output already exists: {output}. "
        ...
        display_name: {
            baseline: {
                metric: _bootstrap_difference(
                    ...
                    seed=20260821 + pool_index * 1000 + index,
                )
...
def main():
    ...
    parser.add_argument(
        "--config", default="research_hsi_confirmation.yaml"
    )
    parser.add_argument(
        "--locked-result-reference",
        default="research_output/hsi_confirmation/confirmation.json",
        help="Original immutable result used to verify a post-hoc audit rerun.",
    )
    parser.add_argument(
        "--frozen-protocol",
        default="research_output/hsi_frozen/frozen_protocol.json",
    )
    parser.add_argument(
        "--freeze-config", default="research_hsi_freeze.yaml"
    )
    parser.add_argument(
        "--output",
        default="research_output/hsi_confirmation/confirmation.json",
