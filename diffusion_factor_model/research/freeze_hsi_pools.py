"""Freeze the Hang Seng Index dual-base pooled protocol with data ending 2023.
...
locked S&P protocol; nothing is tuned to HSI outcomes.
"""
...
def run(args):
    ...
    if output.exists() and not args.allow_overwrite:
        raise FileExistsError(
            f"Frozen HSI protocol already exists: {output}. "
    ...
    seed_reports = []
    for seed in seeds:
        print(f"Freezing HSI dual-pool seed {seed}")
        ...
        checkpoint = checkpoint_dir / f"hsi_frozen_seed{seed}.pt"
    ...
    report = {
        ...
        "protocol": {
            ...
            "hyperparameters_transplanted_from": (
                "locked S&P 500 Phase 2F protocol; no HSI-specific tuning"
            ),
    ...
    print(f"Frozen HSI protocol saved to {output}")
    return report


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default="research_hsi_freeze.yaml")
    parser.add_argument(
        "--output",
