...
| 2026-08-18 | Prospective study design amendments, runner hardening, factor-quality diagnostics, and TSX 60 smoke remediation | Prospective design/development input | Pre-2024 caches and panels only; no post-2023 candidate-market outcomes loaded | Prespecified and implemented before lock: pass/fail/inconclusive claim trichotomy with a terminal program stop rule; operational resume policy with per-market atomic artifacts, attempt log, and evaluation end pinned by the first attempt; post-2023 ticker attrition rule (≥80% session coverage, ≥15 survivors, manifest order, no replacement); raw-download pinning with content hashes; Europe-block (DAX/CAC 40/SMI) sensitivity for both population claims; pre-lock factor-quality diagnostics (mean-factor R², volatility-PCA explained variance). Pre-2024 factor-quality results: DAX R² 0.434 / vol-PCA 62.8%, ASX 200 0.227 / 47.9%, CAC 40 0.433 / 67.4%, NIFTY 50 0.288 / 54.6%, SMI 0.430 / 70.5%, TSX 60 0.304 / 61.6%. During these diagnostics the recorded 2026-08-16 TSX 60 fingerprint failed to reproduce from its pinned pre-2024 cache while all six other markets reproduced exactly. No TSX 60 checkpoint, weight, decision, or post-2023 observation existed, so the screen was re-executed from unchanged caches under unchanged rules; only `screened_at_utc` and the TSX 60 fingerprint changed, eligibility and `K=6` are unchanged, and the superseded artifact is preserved (`pre2024_smoke.superseded-2026-08-16.json`, sha256 3f500de3… matching the committed original). Artifacts: `research_output/prospective_multimarket/pre2024_smoke.json`, `research_output/prospective_multimarket/factor_quality/`. |
| 2026-08-18 | Prospective multi-market freeze and public notarization | Preregistered untouched-market holdout | Six markets trained through 2022; checkpoints and pool weights selected on 2023 validation only; no post-2023 observation loaded | All six eligible markets frozen in one run: DAX (35 assets), ASX 200 (100), CAC 40 (38), NIFTY 50 (46), SMI (18), TSX 60 (57), three seeds each, five-weight grid. Validation selected Student-t-base pool weights of 0 on all CAC 40 seeds and on DAX seeds 42/314, and 0.5 on all TSX 60 seeds. Frozen protocol with checkpoint hashes and prespecified policies committed at `50c7ce7f10c64bc4e62125851d8f21c5268b2436` (tag `multimarket-freeze-2026-08-18`) and publicly pushed to github.com/Roddric/diffusion-model before any post-2023 candidate-market observation was downloaded. Notarization record `research_output/prospective_multimarket/notarization.json` verified against the one-time runner gate (protocol hash, public commit, public URL, pre-evaluation flag). |
| 2026-08-19 | First one-time attempt failed on vendor rate limiting; download transport hardened; no outcome scored | Preregistered untouched-market holdout (operational) | No market scored; the only post-2023 access was a rate-limited transport-failure download, deleted unscored | One-time attempt 1 (evaluation end pinned at 20260818) hit a Yahoo rate-limit storm: all six markets lost their benchmark and nearly all constituents, and the prespecified survivor screen raised the intended transient failure for every market. No market artifacts were written, so the resume policy preserved the pinned evaluation end and permitted retry (`one_time/attempt_log.jsonl`). Remediation: the vendor loader (outside the frozen hash set) now classifies benchmark-missing or wide-missing downloads as transport failures, retries with 60/120/300/600/900/1200-second backoff, and refuses to cache incomplete panels; attempt 1's poisoned extension caches and pinned copies were deleted while the frozen pre-2024 pins were retained. No study scoring semantics changed; pre-2024 panels remain fingerprint-checked at scoring time. Tests: 144 passed. |
| 2026-08-19 | Prospective multi-market one-time evaluation executed — primary and key secondary claims inconclusive; stop rule engaged | Preregistered untouched-market holdout | One-time downloads 2015→2026-08-17 (evaluation end pinned at 20260818 by attempt 1); each market scored exactly once; attempts 2–3 reused scored artifacts without rescoring | Final per-market strong-baseline state ratios: DAX 0.9985 pass, ASX 200 0.9868 pass, CAC 40 1.0074 fail (validation had selected zero diffusion weight on all seeds), NIFTY 50 0.9755 pass, SMI 0.9889 pass, TSX 60 1.0065 fail. Primary population claim: 4 of 6 individual passes against the required 5; geometric-mean ratio 0.99385 with 95% market-t CI [0.98085, 1.00703]; prespecified classification inconclusive. Key secondary covariance: 3 of 6 passes; ratio 0.99073 [0.95016, 1.03304]; inconclusive. Europe-block sensitivity state CI [−0.03008, 0.01335] on the log scale. Horizon 20−5 ratio 0.99253 [0.97780, 1.00748], volatility-regime high−low 0.99658 [0.97844, 1.01506], cross-sectional size slope −6.7e−5 per asset (p=0.775): no moderator detected. Post-2023 ticker attrition dropped zero frozen assets in every market; ASX 200's 29 unresolved download tickers all lay outside the frozen top-100 cross-section. Attempt 2 failed only on TSX 60 via a retry-merge bug on flaky Toronto responses (fixed with regression tests; attempt 3 reused the five scored artifacts and rescored only TSX 60). Per the prespecified stop rule the outcome is terminal for the Student-t-base pool candidate. Artifacts: `research_output/prospective_multimarket/evaluation.json`, `one_time/`. |
| 2026-08-19 | Final research route opened: design frozen, Phase A diagnostics executed, Phase B oracle attribution executed — proceed gate PASSED | Final-route pre-2024 diagnostics | Observations through 2023-12-29 only; post-2023 outcomes read as descriptive labels in Phase A and influence nothing | `FINAL_RESEARCH_ROUTE_DESIGN.md` frozen before execution: pre-2024 data only, no holdout consumption, Phase B proceed gate prespecified, Phase C development rules and the terminal program stop rule fixed. Phase A (hypothesis-generating, K=6): validation-selected pool weight does not rank post-2023 outcomes (TSX 60 selected 0.50 and failed; DAX 0.08 and passed); strongest descriptive correlates of the post-2023 state ratio were session count (ρ +0.71) and mean-factor R² (+0.66). Phase B cross-market oracle-ceiling attribution with the frozen candidate on February–December 2023 development origins (8 origins per market, 3 seeds, 20 paths, 8 coalitions): per-market mean-state/innovation shares DAX 0.482/0.456, ASX 200 0.333/0.573, CAC 40 0.430/0.476, NIFTY 50 0.417/0.493, SMI 0.411/0.507, TSX 60 0.384/0.513; pooled mean-state 0.410 [0.358, 0.461], volatility-state 0.087 [0.073, 0.102], innovation 0.503 [0.461, 0.545]; mean-state + innovation 0.913 [0.898, 0.927] with 6/6 markets above the 0.50 floor. Gate passed all three prespecified criteria (point ≥ 0.80; interval lower > 0.60; ≥5 markets above floor). Representation-drift diagnostic: mean-loading cross-fit/fixed ratios 1.004–1.021 (no exploitable headroom), volatility-loading ratios 0.825–0.927 (7–17% headroom on a component worth 8.7% of loss). Phase C candidate development is permitted under the frozen rules; the diagnostics indicate the substantive target is innovation structure, not loading drift. Artifacts: `FINAL_RESEARCH_ROUTE_DESIGN.md`, `research_output/prospective_multimarket/when_does_it_work/`, `research_output/prospective_multimarket/oracle_attribution/`. |
