...
| 2026-08-15 | State-conditional fixed-candidate robustness freeze — **registered; not yet executed** | Post-selection robustness protocol | Pre-2024 S&P development origins only | Freeze `k=32`; prohibit retuning; compare with Independent-GARCH and unconditional full-vector resampling at 20/50/100 paths, five 20-path repetitions, training-defined volatility regimes, direct realized-covariance losses, and 5% VaR diagnostics. All ten recorded survival requirements must pass before an untouched-market preregistration is allowed. Protocol: `research_output/sp500/state_conditional_robustness.protocol.json`. |
